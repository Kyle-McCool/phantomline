...
"""
    ...
    for item in _extract_json_array(raw):
        ...
        if len(ideas) >= 6:
            break
    if not ideas:
        return jsonify({"ok": False, "error": "AI did not return usable ideas. Try again."}), 502
...
def api_title_ideas():
    ...
    for item in _extract_json_array(raw):
        ...
        if len(titles) >= 12:
            break
    if not titles:
        return jsonify({"ok": False, "error": "AI did not return usable titles. Try again."}), 502
...
def api_tts_cloud():
    ...
    async def _synthesize():
        ...
        # can be anchored to actual speech instead of estimated from chars,
        # which drifts over the course of a long narration.
        boundaries = []
        async for chunk in comm.stream():
            if chunk["type"] == "audio":
                ...
                boundaries.append([
                    round(chunk["offset"] / 10_000_000, 3),
                    round(chunk["duration"] / 10_000_000, 3),
                    chunk["text"],
                ])
        buf.seek(0)
